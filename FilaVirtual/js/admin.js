...
function cargarCasillasUsuario() {
    ...
    // DEBUG: Verificar estado de casillas y usuarios
    console.log('=== DEBUG: ESTADO DE CASILLAS Y USUARIOS ===');
    const todasLasCasillas = db.getCasillas();
    const todosLosUsuarios = db.getUsuarios();
    
    console.log('Todas las casillas:', todasLasCasillas);
    ...
    todasLasCasillas.forEach(casilla => {
        console.log(`Casilla "${casilla.nombre}" (ID: ${casilla.id}):`, {
            ...
            usuariosNombres: (casilla.usuarios || []).map(id => {
                const user = todosLosUsuarios.find(u => u.id === id);
                return user ? user.nombre : `Usuario ${id} (no encontrado)`;
            }),
    ...
    if (db.esDesarrolladorGlobal(usuario.id)) {
        // Desarrollador global ve todas las casillas activas
        casillasDisponibles = casillas.filter(c => c.activa !== false);
        console.log('Desarrollador global - Casillas disponibles:', casillasDisponibles);
    } else {
        ...
        // TEMPORAL: Si el usuario no tiene casillas asignadas, asignar la primera casilla disponible
        if (casillasDisponibles.length === 0 && casillas.filter(c => c.activa !== false).length > 0) {
            console.log('⚠️ Usuario sin casillas asignadas. Asignando casilla automáticamente...');
            const primeraCasillaActiva = casillas.find(c => c.activa !== false);
            if (primeraCasillaActiva) {
                try {
                    db.asignarUsuariosACasilla(primeraCasillaActiva.id, [usuario.id]);
                    console.log(`✅ Casilla "${primeraCasillaActiva.nombre}" asignada automáticamente al usuario "${usuario.nombre}"`);
                    
                    // Recargar casillas disponibles
                    const casillasUsuarioActualizadas = db.obtenerCasillasDeUsuario(usuario.id);
                    const casillasIdsUsuarioActualizadas = casillasUsuarioActualizadas.map(c => c.id);
                    casillasDisponibles = casillas.filter(c => 
                        casillasIdsUsuarioActualizadas.includes(c.id) && c.activa !== false
                    );
                    console.log('Casillas disponibles después de asignación automática:', casillasDisponibles);
                } catch (error) {
                    console.error('❌ Error al asignar casilla automáticamente:', error);
                }
            }
        }
...
function forzarRecargaCasillas() {
    ...
    // Verificar si es desarrollador global
    const esDesarrolladorGlobal = db.esDesarrolladorGlobal(usuario.id);
    console.log('¿Es desarrollador global?', esDesarrolladorGlobal);
    
    // Obtener casillas según el tipo de usuario
    let casillasDisponibles = [];
    
    if (esDesarrolladorGlobal) {
        casillasDisponibles = db.getCasillas().filter(c => c.activa !== false);
        console.log('Desarrollador global - Todas las casillas activas:', casillasDisponibles);
    } else {
        const casillasUsuario = db.obtenerCasillasDeUsuario(usuario.id);
        console.log('Usuario normal - Casillas asignadas:', casillasUsuario);
