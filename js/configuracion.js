document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof db !== 'undefined') {
        console.log('Servicios en BD:', db.getServicios());
        console.log('Casillas en BD:', db.getCasillas());
        console.log('Usuarios en BD:', db.getUsuarios());
        console.log('Datos completos de BD:', localStorage.getItem('turnoDB'));
    } else {
    ...
    setTimeout(() => {
        ...
        // Probar agregar un servicio de prueba si no hay servicios
        if (db.getServicios().length === 0) {
            console.log('No hay servicios, agregando uno de prueba...');
            try {
                db.agregarServicio('Servicio de Prueba', 'Servicio creado automáticamente');
                console.log('Servicio de prueba agregado');
            } catch (error) {
                console.error('Error al agregar servicio de prueba:', error);
            }
        }
        
        // Probar agregar una casilla de prueba si no hay casillas
        if (db.getCasillas().length === 0) {
            console.log('No hay casillas, agregando una de prueba...');
            try {
                const servicios = db.getServicios();
                if (servicios.length > 0) {
                    db.agregarCasilla('Casilla de Prueba', [servicios[0].id]);
                    console.log('Casilla de prueba agregada');
                }
            } catch (error) {
                console.error('Error al agregar casilla de prueba:', error);
            }
        }
        
        // Probar agregar un usuario de prueba si solo hay el desarrollador global
        if (db.getUsuarios().length === 1) {
            console.log('Solo hay el desarrollador global, agregando un usuario de prueba...');
            try {
                db.agregarUsuario('Usuario de Prueba', 'usuario@cesun.edu.mx', 'password123', 'normal');
        ...
        if (typeof cargarServicios === 'function') {
            ...
            console.error('cargarServicios no está definida');
        }
        if (typeof cargarCasillas === 'function') {
            cargarCasillas();
        } else {
            console.error('cargarCasillas no está definida');
        }
        if (typeof cargarUsuarios === 'function') {
            cargarUsuarios();
        } else {
            console.error('cargarUsuarios no está definida');
        }
        cargarFondoGuardado();
    }, 100);
...
function cargarServicios() {
    console.log('Ejecutando cargarServicios...');
    const servicios = db.getServicios();
...
function cargarCasillas() {
    console.log('Ejecutando cargarCasillas...');
    const casillas = db.getCasillas();
    console.log('Casillas obtenidas:', casillas);
    const tabla = document.getElementById('tablaCasillas');
    console.log('Tabla encontrada:', tabla);
    tabla.innerHTML = casillas.map(casilla => {
        const servicios = db.getServicios().filter(s => casilla.servicios && casilla.servicios.includes(s.id));
        ...
        return `
            <tr>
                ...
                <td>
                    <span class="badge ${(casilla.estado || 'libre') === 'libre' ? 'bg-success' : 'bg-danger'}">
                        ${(casilla.estado || 'libre') === 'libre' ? 'Libre' : 'Ocupada'}
                    </span>
                </td>
                <td>
                ...
                <td>
                    <span class="badge ${(casilla.activa !== false) ? 'bg-success' : 'bg-secondary'}">
                        ${(casilla.activa !== false) ? 'Activa' : 'Inactiva'}
                    </span>
                </td>
                <td>
                    <div class="d-flex gap-2">
                        <button onclick="${(casilla.activa !== false) ? 'desactivarCasilla' : 'activarCasilla'}(${casilla.id})" 
                                class="btn btn-sm ${(casilla.activa !== false) ? 'btn-outline-warning' : 'btn-outline-success'}" 
                                title="${(casilla.activa !== false) ? 'Desactivar' : 'Activar'}">
                            <i class="fas ${(casilla.activa !== false) ? 'fa-pause' : 'fa-play'}"></i>
                        </button>
...
function abrirModalUsuariosCasilla(casillaId) {
    ...
    // Cargar usuarios disponibles (solo los visibles según permisos)
    const usuarioActual = auth.getCurrentUser();
    const usuarios = db.getUsuariosVisibles(usuarioActual?.id);
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            ${(() => {
                                ...
                                if (usuario.permisos === 'desarrollador') {
                                    ...
                                    badgeClass = 'bg-info';
                                    permisoTexto = 'Administrador';
                                }
                                
                                return `<span class="badge ${badgeClass}">${permisoTexto}</span>
...
function verUsuariosCasilla(casillaId) {
    ...
    const usuarioActual = auth.getCurrentUser();
    const usuariosVisibles = db.getUsuariosVisibles(usuarioActual?.id);
    const usuarios = usuariosVisibles.filter(u => casilla.usuarios && casilla.usuarios.includes(u.id));
...
function verCasillasUsuario(usuarioId) {
    const usuarioActual = auth.getCurrentUser();
    const usuariosVisibles = db.getUsuariosVisibles(usuarioActual?.id);
    const usuario = usuariosVisibles.find(u => u.id === usuarioId);
    if (!usuario) {
        showAlert('Usuario no encontrado o no tienes permisos para verlo', 'danger');
        return;
    }
...
function cargarUsuarios() {
    ...
    const usuarioActual = auth.getCurrentUser();
    const usuarios = db.getUsuariosVisibles(usuarioActual?.id);
    console.log('Usuarios visibles obtenidos:', usuarios);
    const tabla = document.getElementById('tablaUsuarios');
    console.log('Tabla encontrada:', tabla);
    const esDesarrollador = usuarioActual && usuarioActual.permisos === 'desarrollador';
    
    tabla.innerHTML = usuarios.map(usuario => {
        const esDesarrolladorGlobal = db.esDesarrolladorGlobal(usuario.id);
        const sePuedeEliminar = db.sePuedeEliminarUsuario(usuario.id);
        const sePuedeEliminarDesarrollador = db.sePuedeEliminarDesarrollador(usuario.id);
        const puedeModificar = db.puedeModificarUsuario(usuarioActual?.id, usuario.id);
        // Solo los desarrolladores pueden eliminar al desarrollador global y verificar que no sea el último desarrollador
        const puedeEliminar = sePuedeEliminar && sePuedeEliminarDesarrollador && (esDesarrollador || !esDesarrolladorGlobal);
            ...
            <tr>
                ...
                <td>
                    ${(() => {
                        ...
                        return `<span class="badge ${badgeClass}">${permisoTexto}</span>
                                ${esDesarrolladorGlobal ? '<br><small class="text-warning"><i class="fas fa-shield-alt"></i> Desarrollador Global</small>' : ''}`;
                    })()}
                </td>
                <td>
                ...
                <td>
                    <div class="d-flex gap-2">
                        ...
                        <button onclick="editarUsuario(${usuario.id})" class="btn btn-sm btn-outline-primary" title="Editar" ${!puedeModificar ? 'disabled' : ''}>
                            <i class="fas fa-edit"></i>
                        </button>
                        <button onclick="toggleBloqueoUsuario(${usuario.id})" class="btn btn-sm ${usuario.bloqueado ? 'btn-outline-success' : 'btn-outline-warning'}" title="${usuario.bloqueado ? 'Desbloquear' : 'Bloquear'}" ${!puedeModificar ? 'disabled' : ''}>
                            <i class="fas ${usuario.bloqueado ? 'fa-unlock' : 'fa-lock'}"></i>
                        </button>
                        <button onclick="restablecerContrasenaUsuario(${usuario.id})" class="btn btn-sm btn-outline-warning" title="Restablecer contraseña" ${!puedeModificar ? 'disabled' : ''}>
...
function abrirModalAgregarUsuario() {
    ...
    // Configurar opciones de rol según permisos del usuario actual
    const usuarioActual = auth.getCurrentUser();
    const rolSelect = document.getElementById('usuarioRol');
    
    // Limpiar opciones existentes
    rolSelect.innerHTML = '<option value="normal">Usuario Normal</option>';
    
    // Agregar opciones según permisos
    if (usuarioActual.permisos === 'desarrollador') {
        rolSelect.innerHTML += '<option value="coordinador">Coordinador</option>';
        rolSelect.innerHTML += '<option value="desarrollador">Desarrollador</option>';
    } else if (usuarioActual.permisos === 'coordinador') {
        rolSelect.innerHTML += '<option value="coordinador">Coordinador</option>';
        // Los coordinadores no pueden crear desarrolladores
    }
...
function editarUsuario(usuarioId) {
    ...
    const usuarioActual = auth.getCurrentUser();
    if (!db.puedeModificarUsuario(usuarioActual?.id, usuarioId)) {
        showAlert('No tienes permisos para modificar este usuario', 'danger');
        return;
    }
...
function guardarUsuario() {
    ...
    // Validar que el usuario actual puede crear usuarios con el rol seleccionado
    const usuarioActual = auth.getCurrentUser();
    if (rol === 'desarrollador' && usuarioActual.permisos !== 'desarrollador') {
        showAlert('Solo los desarrolladores pueden crear usuarios con permisos de desarrollador', 'danger');
        return;
    }
    
    if (rol === 'coordinador' && !['coordinador', 'desarrollador'].includes(usuarioActual.permisos)) {
        showAlert('Solo los coordinadores y desarrolladores pueden crear usuarios coordinadores', 'danger');
        return;
    }
    ...
    try {
        if (id) {
            ...
            const usuarioActual = auth.getCurrentUser();
            if (!db.puedeModificarUsuario(usuarioActual?.id, parseInt(id))) {
                showAlert('No tienes permisos para modificar este usuario', 'danger');
                return;
            }
            
            // Validar que el usuario actual puede asignar el rol seleccionado
            if (rol === 'desarrollador' && usuarioActual.permisos !== 'desarrollador') {
                showAlert('Solo los desarrolladores pueden asignar permisos de desarrollador', 'danger');
                return;
            }
            
            if (rol === 'coordinador' && !['coordinador', 'desarrollador'].includes(usuarioActual.permisos)) {
                showAlert('Solo los coordinadores y desarrolladores pueden asignar permisos de coordinador', 'danger');
                return;
            }
...
function toggleBloqueoUsuario(usuarioId) {
    ...
    try {
        db.setBloqueoUsuario(usuarioId, !usuario.bloqueado);
...
function eliminarUsuario(usuarioId) {
    ...
    const usuarioActual = auth.getCurrentUser();
    if (!db.puedeModificarUsuario(usuarioActual?.id, usuarioId)) {
        showAlert('No tienes permisos para eliminar este usuario', 'danger');
        return;
    }
    
    // Verificar si se puede eliminar el desarrollador
    if (!db.sePuedeEliminarDesarrollador(usuarioId)) {
        const desarrolladores = db.getUsuarios().filter(u => u.permisos === 'desarrollador');
        showAlert(`No se puede eliminar el último desarrollador del sistema. Actualmente hay ${desarrolladores.length} desarrollador(es). Debe existir al menos un desarrollador.`, 'danger');
        return;
    }
...
window.guardarCasilla = guardarCasilla;