...
class TurnoDB {
    constructor() {
        ...
        // Crear usuario desarrollador por defecto si no hay usuarios
        if (this.usuarios.length === 0) {
            this.agregarUsuario('Luis Mejia', 'soportetecnico@cesun.edu.mx', 'C3sun2025*', 'desarrollador');
        }
    ...
    cargarDatos() {
        ...
        // Migrar usuarios existentes con permisos 'admin' a 'coordinador'
    ...
    migrarUsuariosExistentes() {
        // Migrar usuarios con permisos 'admin' a 'coordinador' (excepto el desarrollador global)
        this.usuarios.forEach(usuario => {
            if (usuario.permisos === 'admin' && usuario.correo !== 'soportetecnico@cesun.edu.mx') {
                usuario.permisos = 'coordinador';
                console.log(`Usuario migrado de admin a coordinador: ${usuario.nombre} (${usuario.correo})`);
            }
        });
        
        // Asegurar que el usuario global sea desarrollador
        const usuarioGlobal = this.usuarios.find(u => u.correo === 'soportetecnico@cesun.edu.mx');
        if (usuarioGlobal && usuarioGlobal.permisos !== 'desarrollador') {
            usuarioGlobal.permisos = 'desarrollador';
            console.log(`Usuario global migrado a desarrollador: ${usuarioGlobal.nombre}`);
        }
    ...
    verificarPermisos(usuarioId, permisoRequerido) {
        ...
        if (permisoRequerido === 'admin') {
            // Ahora admin incluye tanto coordinadores como desarrolladores
            return usuario.permisos === 'coordinador' || usuario.permisos === 'desarrollador';
        }
        
        if (permisoRequerido === 'coordinador') {
            return usuario.permisos === 'coordinador' || usuario.permisos === 'desarrollador';
        }
        
        if (permisoRequerido === 'desarrollador') {
            return usuario.permisos === 'desarrollador';
        }
        
        return true;
    }

    // Verificar si un usuario es el desarrollador global (no se puede eliminar)
    esDesarrolladorGlobal(usuarioId) {
        const usuario = this.getUsuario(usuarioId);
    ...
    sePuedeEliminarUsuario(usuarioId) {
        ...
        // El desarrollador global no se puede eliminar
    ...
    puedeModificarUsuario(usuarioModificadorId, usuarioAModificarId) {
        const usuarioModificador = this.getUsuario(usuarioModificadorId);
        const usuarioAModificar = this.getUsuario(usuarioAModificarId);
        
        if (!usuarioModificador || !usuarioAModificar) {
            return false;
        }
        
        // Los desarrolladores pueden modificar a cualquiera
        if (usuarioModificador.permisos === 'desarrollador') {
            return true;
        }
        
        // Los coordinadores pueden modificar usuarios normales y otros coordinadores, pero no desarrolladores
        if (usuarioModificador.permisos === 'coordinador') {
            return usuarioAModificar.permisos !== 'desarrollador';
        }
        
        // Los usuarios normales no pueden modificar a nadie
        return false;
    }

    // Verificar si se puede eliminar un usuario desarrollador
    sePuedeEliminarDesarrollador(usuarioId) {
        const usuario = this.getUsuario(usuarioId);
        if (!usuario || usuario.permisos !== 'desarrollador') {
            return true; // No es desarrollador, se puede eliminar normalmente
        }
        
        // Contar cuántos desarrolladores hay
        const desarrolladores = this.usuarios.filter(u => u.permisos === 'desarrollador');
        
        // Si solo hay uno, no se puede eliminar
        if (desarrolladores.length === 1) {
            return false;
        }
    ...
    // Obtener usuarios visibles según el tipo de usuario que los está viendo
    getUsuariosVisibles(usuarioId) {
        const usuarioActual = this.getUsuario(usuarioId);
        if (!usuarioActual) {
            return [];
        }
        
        // Los desarrolladores ven todos los usuarios
        if (usuarioActual.permisos === 'desarrollador') {
            return this.usuarios;
        }
        
        // Los coordinadores ven todos excepto desarrolladores
        if (usuarioActual.permisos === 'coordinador') {
            return this.usuarios.filter(u => u.permisos !== 'desarrollador');
        }
        
        // Los usuarios normales no ven la lista de usuarios
        return [];
    }
